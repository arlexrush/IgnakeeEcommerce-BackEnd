...
namespace Ecommerce.Application.Features.ShoppingCarts.Vms
{
    public class ShoppingCartVm
    {
        ...
        {
            get { return (Math.Round((Items!.Sum(x => x.Price * x.Quantity)), 2)); }
            set {; }
        }

        public decimal Tax
        {
            get { return Math.Round(((Items!.Sum(x => x.Price * x.Quantity)) * (Convert.ToDecimal(0.18))), 2); }
            set {; }
        }

        public decimal Shipping
        {
            get { return Math.Round(((Items!.Sum(x => x.Price * x.Quantity)) < 100 ? 10m : 25m), 2); }
            set { }
        }
